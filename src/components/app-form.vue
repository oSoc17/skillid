<template>
  <div id="form">
    <form id="app-form" class="form">
      ...
      <nav id="navigation">
        <ol class="nav-list">
          <li v-bind:class="{actif: formControlElements.searchActif}">{{formContentElements.searchNavLabel}}</li>
          <li v-bind:class="{actif: formControlElements.metaDataActif}">{{formContentElements.metaDataNavLabel}}</li>
          <li v-bind:class="{actif: formControlElements.personalizeActif}">{{formContentElements.personalizeNavLabel}}</li>
        </ol>
      </nav>
      ...
      <div class = "search-content" v-show='formControlElements.searchActif'>
        ...
        <div class="">
          ...
          <button class="search-button" type="button" name="button">{{formContentElements.searchButton}}</button>
  			...
  			<ol class="list">
          <li v-for="searchResult in formContentValues.searchResults">
            {{ searchResult }}
          </li>
        </ol>
      </div>
      ...
      <div id="personalize-content" v-show='formControlElements.personalizeActif'>
        <div class="personalize-content-labels">
          <label v-on:click.prevent="changeStateInputField('imageInputActif')" for="company-logo">{{formContentElements.companyLogoLabel}} <br><img class="label-icon" src="../assets/svg/company.svg" alt="" ></label>
        ...
        <div class="personalize-content-inputs">
          <div class="input-field" v-bind:class="{hiddenInput: formControlElements.imageInputActif}">
